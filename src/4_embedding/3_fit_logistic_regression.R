# 4_fit_logistic_regression.R
# Fit logistic regression on PCA-reduced embedding distributions to predict censorship
...
cat("============================================================\n")
cat("EMBEDDING LOGISTIC REGRESSION MODEL\n")
cat("============================================================\n")

# Create artifacts directory
if (!dir.exists("artifacts/4_embedding")) {
  dir.create("artifacts/4_embedding", recursive = TRUE)
}
...
# -----------------------------------------------------------------------------
# 1. Check embedding distributions table exists and get component count
# -----------------------------------------------------------------------------

cat("\nChecking embedding distributions table...\n")

table_check <- dbGetQuery(conn,
  "SELECT name FROM sqlite_master WHERE type='table' AND name='embedding_distributions'")

if (nrow(table_check) == 0) {
  stop("embedding_distributions table not found. Please run 3_reduce_embeddings.py first.")
}

# Get component columns
emb_cols <- dbListFields(conn, "embedding_distributions")
comp_cols <- emb_cols[grepl("^emb_", emb_cols)]
n_components <- length(comp_cols)

cat("Found", n_components, "embedding components\n")

# Get row counts
emb_count <- dbGetQuery(conn, "SELECT COUNT(*) as n FROM embedding_distributions")$n
all_count <- dbGetQuery(conn, "SELECT COUNT(*) as n FROM all_data WHERE permission_denied IS NOT NULL")$n

cat("Embedding distributions:", format(emb_count, big.mark=","), "rows\n")
...
dbExecute(conn, "DROP VIEW IF EXISTS embedding_censorship")

view_sql <- paste0("
  CREATE VIEW embedding_censorship AS
  SELECT
    a.permission_denied,
    ", paste(paste0("e.", comp_cols), collapse = ", "), "
  FROM all_data a
  INNER JOIN embedding_distributions e ON a.ROWID = e.rowid
...
# Verify view
view_count <- dbGetQuery(conn, "SELECT COUNT(*) as n FROM embedding_censorship")$n
...
# Build formula
predictors <- comp_cols
response <- "permission_denied"
formula <- make.formula(response, predictors)

cat("Formula: permission_denied ~", n_components, "embedding components\n")

# Create data function for chunked processing
data_fun <- make.data(
  response = response,
  predictors = predictors,
  table = "embedding_censorship",
...
cat("\nTop 10 components by effect size:\n")
print(head(coef_df, 10))

# Significant predictors
sig_preds <- get_sigpreds(model)

cat("\nSignificant components making censorship MORE likely:", nrow(sig_preds$more_likely), "\n")
cat("Significant components making censorship LESS likely:", nrow(sig_preds$less_likely), "\n")

# Save coefficients
write.csv(coef_df, "artifacts/4_embedding/embedding_coefficients.csv", row.names = FALSE)
write.csv(sig_preds$more_likely, "artifacts/4_embedding/embedding_positive_predictors.csv", row.names = FALSE)
write.csv(sig_preds$less_likely, "artifacts/4_embedding/embedding_negative_predictors.csv", row.names = FALSE)

# Save model summary
sink("artifacts/4_embedding/embedding_model_summary.txt")
print(model_summary)
cat("\nAIC:", model_aic, "\n")
cat("BIC:", model_bic, "\n")
...
saveRDS(list(
  ...
  aic = model_aic,
  bic = model_bic
), file = "artifacts/4_embedding/embedding_model.rds")

cat("\nModel saved to artifacts/4_embedding/embedding_model.rds\n")
...
sample_query <- paste0("
  SELECT permission_denied, ", paste(comp_cols, collapse = ", "), "
  FROM embedding_censorship
  ORDER BY RANDOM()
  LIMIT ", sample_size
)
...
write.csv(binned_res, "artifacts/4_embedding/binned_residuals.csv", row.names = FALSE)
...
p1 <- ggplot(binned_res, aes(x = predicted, y = residuals)) +
  ...
  labs(title = "Binned Residuals vs Predicted Values",
       subtitle = "Embedding Model (PCA-reduced)",
...
ggsave("artifacts/4_embedding/binned_residuals_plot.png", plot = p1,
       width = 10, height = 6, dpi = 150)
cat("Saved: artifacts/4_embedding/binned_residuals_plot.png\n")
...
png("artifacts/4_embedding/roc_curve.png", width = 800, height = 600, res = 150)
plot(roc_obj, main = paste("ROC Curve - Embedding Model (AUC =", round(auc_value, 3), ")"),
     col = "blue", lwd = 2)
abline(a = 0, b = 1, lty = 2, col = "gray")
dev.off()
cat("Saved: artifacts/4_embedding/roc_curve.png\n")

# Plot 3: Coefficient plot (top 20 by effect size)
coef_plot_df <- coef_df[coef_df$predictor != "(Intercept)", ]
coef_plot_df <- head(coef_plot_df, 20)  # Top 20
...
p3 <- ggplot(coef_plot_df, aes(x = estimate, y = predictor)) +
  ...
  labs(title = "Top 20 Embedding Component Coefficients",
...
ggsave("artifacts/4_embedding/coefficient_plot.png", plot = p3,
       width = 10, height = 8, dpi = 150)
cat("Saved: artifacts/4_embedding/coefficient_plot.png\n")
...
ggsave("artifacts/4_embedding/pearson_residuals_hist.png", plot = p4,
       width = 10, height = 6, dpi = 150)
cat("Saved: artifacts/4_embedding/pearson_residuals_hist.png\n")
...
fit_stats <- data.frame(
  metric = c("AIC", "BIC", "AUC", "Accuracy", "Precision", "Recall", "F1_Score",
             "Dispersion", "N_observations", "N_components",
  ...
  value = c(model_aic, model_bic, auc_value, accuracy, precision, recall, f1_score,
            dispersion, model$n, n_components,
            mean(raw_residuals, na.rm = TRUE), sd(raw_residuals, na.rm = TRUE),
            sum(abs(binned_res$residuals) > 2 * binned_res$ellipse))
)

write.csv(fit_stats, "artifacts/4_embedding/model_fit_statistics.csv", row.names = FALSE)
cat("\nSaved: artifacts/4_embedding/model_fit_statistics.csv\n")
...
write.csv(residual_df, "artifacts/4_embedding/residuals_sample.csv", row.names = FALSE)
cat("Saved: artifacts/4_embedding/residuals_sample.csv\n")
...
cat("\n============================================================\n")
cat("EMBEDDING LOGISTIC REGRESSION COMPLETE\n")
cat("============================================================\n")
cat("Results saved to artifacts/4_embedding/\n")
