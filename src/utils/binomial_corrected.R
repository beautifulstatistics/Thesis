...
aic.binomial<-function(y, n, mu, wt, dev){
  loglike_corrected(y, n, mu, wt, dev)
}
...
shglm2 <- function (formula, datafun, family = gaussian(), weights.fo = NULL, 
...
{
  if (!is.null(start)) 
    stop("Sorry, code for argument start is not implemented yet")
  if (!is.null(mustart)) 
    stop("Sorry, code for argument mustart is not implemented yet")
  if (!is.null(etastart)) 
    stop("Sorry, code for argument etastart is not implemented yet")
  ...
  fa <- NULL
  if (!all.levels) {
    fa <- which(attributes(attributes(M)$terms)$dataClasses == 
                  "factor")
    if (length(fa) > 0) {
  ...
  weights <- if (is.null(weights.fo)) 
    rep(1, nobs)
  else model.frame(weights.fo, dati)[[1]]
  ...
  while ((tol > set$acc) & (iter < maxit)) {
    ...
    while (!eof) {
      ...
      if (iter > 1) {
        ...
        if (nvar == 1) {
          ...
          eta <- (offset + if (sparse) 
            drop(X[, ok] %*% start)
            else drop(tcrossprod(X[, ok], t(start))))[1:length(y)]
        }
        mu <- linkinv(eta <- eta)
      }
      else {
        ...
        if (is.null(start)) {
          ...
          if (nvar == 1) {
            ...
            eta <- (offset + if (sparse) 
              drop(X %*% start)
              else as.vector(tcrossprod(X, t(start))))[1:length(y)]
          }
          mu <- linkinv(eta)
        }
      }
      if (trace) 
        cat("iter", iter, "elaborating chunk", iter2, 
            "dim", dim(X), "\n")
      dev <- dev + sum(dev.resids(y, c(mu), weights))
      if (iter > 1) 
        aic.model <- aic.model + aic.shglm(fam, y, wt, 
                                           mu, weights, dev.prec)
      varmu <- variance(mu)
      ...
      if (iter2 == 1) {
        ...
        XTX[rownames(Ax), colnames(Ax)] <- XTX[rownames(Ax), 
                                               colnames(Ax)] + Ax
        Az <- XTz
        XTz <- if (sparse) 
          t(X) %*% (W * z)
        else t(crossprod((W * z), X))
        XTz[rownames(Az), ] <- XTz[rownames(Az), ] + 
          Az
      }
      res <- (y - mu)/mu.eta(eta)
      ...
      if (is.null(dati <- datafun(reset = FALSE))) {
        if (iter == 1) {
          ris <- if (set$eigendec) 
            control(XTX, , set$tol.values, set$tol.vectors, 
                    out.B = FALSE, set$method)
      ...
      if (iter == 1) {
        ...
        zero.weights <- zero.weights + sum(weights == 
                                             0)
      ...
      if (iter == 2) {
        ...
        nulldev <- nulldev + sum(dev.resids(y, c(wtdmu), 
                                            weights))
      }
    }
  }
  ...
  rval <- list(coefficients = coefficients, logLik = ll.nuovo, 
               ...
               df = resdf, XTX = XTX[ok, ok], dispersion = dispersion, 
               nok = ris$nok, ok = ok, RSS = RSS, ncoll = ris$ncoll, 
               sparse = sparse, nulldev = nulldev, rank = rank, deviance = dev, 
